...
def read_morpho_train_set_synth():
    word_list = []
    word_segmentation = []
    with open("./data_sources/morpho_train_set_synth.tsv") as file:
        tsv_file = csv.reader(file, delimiter="\t")
        for line in tsv_file:
            word_list.append(line[0])
            word_segmentation.append(line[1])
...
def read_sigmorphon_train_set():
    word_list = []
    word_segmentation = []
    with open("./data_sources/sigmorphon/ces.word.train.tsv") as file:
        tsv_file = csv.reader(file, delimiter="\t")
        for line in tsv_file:
            word_list.append(line[0])
            word_segmentation.append(line[1].replace(' @@', '-'))
    return word_list, word_segmentation


def read_sigmorphon_dev_set():
    word_list = []
    word_segmentation = []
    with open("./data_sources/sigmorphon/ces.word.dev.tsv") as file:
        tsv_file = csv.reader(file, delimiter="\t")
        for line in tsv_file:
            word_list.append(line[0])
            word_segmentation.append(line[1].replace(' @@', '-'))
...
def read_sigmorphon_test_set():
    word_list = []
    word_segmentation = []
    with open("./data_sources/sigmorphon/ces.word.test.gold.tsv") as file:
        tsv_file = csv.reader(file, delimiter="\t")
        for line in tsv_file:
            word_list.append(line[0])
            word_segmentation.append(line[1].replace(' @@', '-'))
