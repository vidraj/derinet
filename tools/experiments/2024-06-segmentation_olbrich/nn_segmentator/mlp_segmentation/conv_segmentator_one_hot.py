...
def train_and_predict(train_set, dev_set, test_set, limit, input_dim):
    # Create logdir name
    ...
    # hidden_emb = keras.layers.Dense(1500, activation="relu")(input_emb)
    ...
    hidden21 = keras.layers.Conv1D(200, 1, padding="same", activation="relu")(merged)
    ...
    hidden23 = keras.layers.Conv1D(100, 3, padding="same", activation="relu")(merged)
    hidden23 = keras.layers.BatchNormalization()(hidden23)

    hidden24 = keras.layers.Conv1D(100, 4, padding="same", activation="relu")(merged)
    ...
    merged = keras.layers.concatenate([hidden21, hidden22, hidden23, hidden24,hidden25], axis=-1)
    hidden = keras.layers.Flatten()(merged)
    hidden1 = keras.layers.Dense(1200, activation="relu")(hidden)
    ...
    model.fit(train_set[0], train_set[1], epochs=18, batch_size=64, validation_batch_size=64,
              validation_data=(dev_set[0], dev_set[1]) , verbose=1, callbacks=[tb_callback])
