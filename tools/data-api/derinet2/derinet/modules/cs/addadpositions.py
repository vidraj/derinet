...
class AddAdpositions(Block):
    ...
    def process(self, lexicon: Lexicon):
        ...
        This block assumes that everything in the lemma column is an adposition, and adds everything therein
        with the ADP tag into DeriNet.
        ...
        for row in newdf.itertuples():
            ...
            if lexicon.get_lexemes(lemma, pos="ADP"):
                logger.warning(f"Adposition {lemma} already exists in DeriNet, skipping.")
                continue

            logger.debug(f"Adding Adposition '{lemma}'")
