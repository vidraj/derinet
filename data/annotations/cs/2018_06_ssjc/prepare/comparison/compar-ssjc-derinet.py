...
def load_wkt(path): # loading SSJC data
...
def inDeriNet(word): # function given boolean about existence in DeriNet
    ...
    pos = word.split('_')[1]
    try:
        state = der.get_lexeme(node=lexeme)
        return (True, lexeme + '_None')
    except:
        try:
            if (lexeme.endswith('ti')): lexeme = lexeme[:-1]
            state = der.get_lexeme(node=lexeme)
            return (True, lexeme + '_None')
        except:
            return (False, lexeme + '_None')
...
def exist_in_derinet(der, wordlist):
    ...
    for word in wordlist:
        inD = inDeriNet(word)
        if (inD[0] is True):
            if (' ' in word): inside_more.add(inD[1])
            else: inside_one.add(inD[1])
            inside.add(inD[1])
        else:
            if (' ' in word): outside_more.add(inD[1])
            else: outside_one.add(inD[1])
            outside.add(inD[1])
    return inside, outside, inside_more, inside_one, outside_more, outside_one

def parents_for_derinet_root(der, relations):
    # potential parents from SSJC for DeriNet's roots
    ...
    # potential parents from SSJC for DeriNet's root existed in DeriNet
    existedparents = defaultdict(set)
    for root,parents in allparents.items():
        for parent in parents:
            inD = inDeriNet(parent)
            if (inD[0] is True):
                existedparents[root].add(inD[1])
...
def print_existance(path, nameside, data):
    with open(file=path, mode='w', encoding='utf-8') as f:
        f.write('### Number of lexemes in SSJC and ' + nameside + ' DeriNet (all): ' + str(len(data)) + ' ###\n\n')
        for w in sorted(data):
            f.write(w + '\n')

def print_root_parents(path, dic):
    with open(file=path, mode='w', encoding='utf-8') as f:
        f.write('### Number of DeriNets roots with parents in SSJC (in this file): ' + str(len(dic)) + ' ###\n\n')
...
if (__name__ == '__main__'):
    ...
    # loading data
    der = DeriNet(par.d)
    wordlist, relations = load_wkt(par.s)

    # creating and saving lists of SSJC lexemes inside/outside of DeriNet
    if (par.i) or (par.o):
    ...
    # creating and saving lists of DeriNets root having parent(s) in SSJC
