...
ostrava_filename = sys.argv[1]
sigmorphon_filename = sys.argv[2]


ostrava2segmented = {}


fh_ostrava = open (ostrava_filename)
for line in fh_ostrava:
    segmented = line.rstrip()
    unsegmented = segmented.replace('-','')
    ostrava2segmented[unsegmented] = segmented


fh_sigmorphon = open(sigmorphon_filename)

for line in fh_sigmorphon:
    columns = line.rstrip().split('\t')
    (unsegmented, retro, segmented, source) = columns[:4]

    if unsegmented in ostrava2segmented and segmented != ostrava2segmented[unsegmented]:
        print("\t".join([unsegmented, retro, ostrava2segmented[unsegmented], segmented, source]))
